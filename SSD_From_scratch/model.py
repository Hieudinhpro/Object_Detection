...
"""create a ssd model with sub part"""

def create_vgg():
    '''create vgg model base
        init input chanel = 3 and output chanel = 1024
        64,64, 'M' ,128,'MC' ...
        64, 128,...etc is chanel output
        M maxPooling layer
        (option) MC maxPooling layer has ceil_mode (lam tron pixel len)
...
def create_extras():
    '''create model extract feature that get output of last layer VGG model then pass it into extrax model
        init input chanel 1024 because last layer VGG model is 1024
        '''
...
def create_loc_conf(num_classes=21, bbox_aspect_num=[4, 6, 6, 6, 4, 4]):
    '''create loc (locations) and confidence funcition
        locations has shape 1, 8732, 4
        confidence has shape 1, 8732, 21
        8732 is numb bonding boxes
        38 x 38 + 19x19 +10x10 +5x5 +3x3 +1x1 = 1940 bouding boxes 
        more bouding boxes is better ==> increase bouding boxes ( defaults is a bouding box)
        38 x 38 x4 + 19x19x6 +10x10x6 +5x5x6 +3x3x4 +1x1x4 =8732 bouding boxes (x4 or x6 is idea of author )
        '''
...
class SSD(nn.Module):
    ...
    def forward(self, x):
        ...
        for (x, l, c) in zip(sources, self.loc, self.conf): # type: ignore
